...
class NeuralNetwork():
    ...
    def _backpropagate(self, delta):
        ...
        n_layers = len(self.n_hiddens_each_layer) + 1
        if self.debug:
            print('in_backpropagate: first delta calculated is\n{}'.format(delta))
...
class NeuralNetworkClassifier(NeuralNetwork):
    ...
    def _neg_log_likelihood_f(self, X, T):
        ...
        Ys = self._forward(X)
        G = self._softmax(Ys[-1])
        intermediate_value = np.log(G + sys.float_info.epsilon)
        neg_mean_log_likelihood =  - np.mean(T * intermediate_value)
        
        if self.debug:
            print('Result of call to self._forward is:\n{}'.format(Ys))
            print('Result of _softmax is:\n{}'.format(G))
            print('Result of np.log(Y + sys.float_info.epsilon) is:\n{}'.format(intermediate_value))
            print('_neg_log_likelihood_f returns:\n{}'.format(neg_mean_log_likelihood))
    ...
    def use(self, X):
        ...
        classes = self.classes[np.argmax(Y_softmax,axis=1).reshape(-1,1)]
